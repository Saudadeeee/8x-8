...
func _build_ui() -> void:
	...
	var banner_tex = load("res://assets/ui/victory_banner.png") as Texture2D
	if banner_tex:
	...
	subtitle.text = "Your kingdom prevails!"
	subtitle.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	subtitle.add_theme_font_size_override("font_size", 22)
	subtitle.add_theme_color_override("font_color", Color(0.9, 0.9, 0.9, 1))
...
func _on_run_ended(is_victory: bool) -> void:
	if is_victory:
		var gm = get_node_or_null("/root/GameManagerSingleton")
		if gm:
			show_stats(gm.current_wave, gm.run_enemies_killed, gm.run_gold_earned, gm.run_meta_points_earned)
